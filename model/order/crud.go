...
func (m *mysqlCRUD) FindOrdersNotIssue(orderId []string) ([]*dao.Order,error){
	...
	selectColumn := []string{"order.order_id","order.receipt_uniform_no","order.firstname","order.lastname","order.email","order.telephone","order.carrier_type","order.carrier_id1","order.carrier_id2","order.donate_no","order.total"}
